...
The ``THEarlyStopping`` wraps the early stopping implementation of `Keras <https://keras.io/api/callbacks/early_stopping/>`_ to work with any ``Model`` component.

The ``THEarlyStopping`` uses the ``THEarlyStoppingConfig`` as default configuration template

.. code-block:: python

    class THEarlyStoppingConfig(Configuration):
